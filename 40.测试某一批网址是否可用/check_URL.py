...
#  2020-5-15 12:14   技术笔记   1,711   846 字   11 分钟
# 最近有一个批量检测 URL 是否可以访问的需求，于是找了一个 Python 脚本来实现这个需求（ Python 2.7+ 测试有效）。
# https://chaisw.cn/blog/1854.html
# 参考原文
...
def request_url(url):
    ...
    return return_log('HTTP', is_http_urllib_true and is_http_urllib2_true, http_url, http_urllib_log, http_urllib2_log) \
           + '\n' + return_log('HTTPS', is_https_urllib_true and is_https_urllib2_true, https_url, https_urllib_log,
                               https_urllib2_log)
...
def open_thread(thread_total_number, url_of_list, log_of_list):
    ...
    for thread_number in range(thread_total_number):
        thread = threading.Thread(target=use_thread,
                                  args=(thread_total_number, thread_number, url_of_list, log_of_list))
        thread.start()
